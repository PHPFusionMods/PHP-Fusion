			...
			if (check_panel_status($p_side['side'])) {
				foreach ($panels_cache[$p_key + 1] as $p_data) {
					...
					if ($p_data['panel_url_list'] == ""
					    || ($p_data['panel_restriction'] == 1 && !in_array(TRUE_PHP_SELF.(FUSION_QUERY ? "?".FUSION_QUERY : ""), $url_arr))
						|| ($p_data['panel_restriction'] == 0 && in_array(TRUE_PHP_SELF.(FUSION_QUERY ? "?".FUSION_QUERY : ""), $url_arr)))
					{ 
