...
if (str_replace(".", "", $settings['version']) < "70204") {
	if (!isset($_POST['stage'])) {
		...
		$check = dbcount("(settings_name)", DB_SETTINGS, "settings_name='comments_avatar'");
		if (!$check) {
			$result = dbquery("INSERT INTO ".DB_SETTINGS." (settings_name, settings_value) VALUES ('comments_avatar', '1')");
		}
		$check = dbcount("(settings_name)", DB_SETTINGS, "settings_name='multiple_logins'");
		if (!$check) {
			$result = dbquery("INSERT INTO ".DB_SETTINGS." (settings_name, settings_value) VALUES ('multiple_logins', '0')");
		}
		$result = dbquery("UPDATE ".DB_SETTINGS." SET settings_value='7.02.04' WHERE settings_name='version'");
